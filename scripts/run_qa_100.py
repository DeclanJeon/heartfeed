#!/usr/bin/env python3
"""Run 100 questions through v2 pipeline (sequential, incremental save)."""
...
LIMIT = 100
RESULTS_PATH = ROOT / "data" / "eval" / "qa_100_results.jsonl"
...
async def main():
    ...
    remaining = [q for q in questions if q["id"] not in done_ids][:LIMIT]
    total_remaining = len(remaining)
    print(f"Limit: {LIMIT}, Done: {len(done_ids)}, Running: {total_remaining}", flush=True)
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        async with lifespan(app):
            for q in remaining:
                ...
                if count % 10 == 0 or count == total_remaining:
                    elapsed_total = time.perf_counter() - t_start
                    rate = count / elapsed_total
                    eta_min = (total_remaining - count) / rate / 60 if rate > 0 else 0
                    print(
